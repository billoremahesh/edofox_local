        ...
        <tbody>
            ...
            ?>
                <tr>
                    <td><?= $test['id'] ?></td>
                    <td><?= $key + 1 ?></td>
                    <td style="max-width: 200px; color: #5b51d8"><?= $test_name ?></td>
                    <td><?= date("d M Y, H:i A", $test['startDate'] / 1000);  ?></td>
                    <td class="text-muted"><?= $appeared_on; ?></td>
                    <td style="color: #4caf50"><?= $score_text ?></td>
                    <td style="color: #311b92"><?= $rank ?></td>
                    <td>
                        <?php
                        if ($score > 0) {
                            foreach ($test['analysis']['subjectAnalysis'] as $key => $analysis) :
                                $subject_name = $analysis['subject'];
                                $subject_score = $analysis['score'];

                                //Check if subject already added in global array of subjects
                                //If not, add in the chart associative array
                                if (!in_array($subject_name, $all_subjects_array, true)) {
                                    array_push($all_subjects_array, $subject_name);
                                    $subjectwise_chart_data_array["$subject_name"] = array();
                                }
                        ?>
                                <p style="background-color: <?= $subjectsColorArray[$key]; ?>; color: white; display: inline-block; padding: 4px 8px; margin: 4px; border-radius: 20px;"><?= $subject_name ?> : <b><?= $subject_score ?></b></p>
                        <?php

                                //Add subjectwise score data for subjectwise chart
                                //Create an associative array of arrays
                                foreach ($subjectwise_chart_data_array as $key => $subject_scores_array) {
                                    $arrayScore = $test_name . ":" . $subject_score;
                                    if ($key == $subject_name) {
                                        array_push($subjectwise_chart_data_array["$subject_name"], $arrayScore);
                                        //echo $key . " = " . $subject_name . " = " . $subject_score;
                                    }
                                }
