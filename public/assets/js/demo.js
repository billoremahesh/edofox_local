...
*/
    ...
    function uploadFile(blob) {
        ...
        toggleSpinner("show", "Cropping image .. Please wait ..");
        ...
        get_admin_token().then(function (result) {
            ...
            if (resp != null && resp.status == 200 && resp.data != null && resp.data.admin_token != null) {
                ...
                //data.append('fileName', 'Q-' + questionId + '.png');
                data.append('request', JSON.stringify(request));
        
                jQuery.ajax({
                    url: rootAdmin + 'cropQuestionImage',
                    data: data,
                    ...
                    success: function (data) {
                        ...
                        if(data != null && data.status.statusCode == 200) {
                            $("#reviewImage").attr("src", data.question.questionImageUrl);
                            $("#reviewTitle").text("Crop successful for Q " + questionId);
                            $("#reviewQuestion").modal('show');
                        } else {
                            $("#reviewTitle").text("Crop failed for Q " + questionId);
                        }
                        
                    },
                    error: function (error) {
                        console.log("Error!");
                        alert("Error in cropping the image!");
                    }
                });
            }
        });
    ...
    function demoResizer() {
        ...
        document.querySelector('.resizer-result').addEventListener('click', function (ev) {
            ...
            resize.result({
                ...
                // console.log("Done.. size is " + blob.size);
