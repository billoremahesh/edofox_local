...
JQSpeedTest = function(options) {
	...
	function testStart(){
		...
		if(dlCounts < settings.countDlSamples && settings.downloadTest){
		...
		else if(ulCounts < settings.countUlSamples && settings.uploadTest){
			if(dlTestRunning == 'no' && ulTestRunning == 'no' && reTestRunning == 'no'){
				ulCounts++;
				ulTestRunning = 'yes';
	...
	function TestDownload(){
		...
		console.log("Calling upload speed test ajax ..");
		$.ajax({
			type: "GET",
			...
			error: function(error) {
				...
				typeof settings.testDlCallback === 'function' && settings.testDlCallback(-1, 0);
				typeof settings.testUlCallback === 'function' && settings.testUlCallback(-1, 0);
			}
		});
	}
	...
	function TestUpload(){
		...
		$.ajax({
			...
			error: function(error) {
				console.log("Error in upload speed test", error);
				typeof settings.testUlCallback === 'function' && settings.testUlCallback(-1, 0);
			}
		});
	}
